...
__Data Export__
The following code chunk excludes variables that are not necessary for the Shiny app to run and saves the data in format appropriate for shiny. Gender is recoded\ as a factor for more readable filtering in shiny and the two time variables are also recoded as POSIXct/POSIXt variables for easier selection and filtering.
```{r}
week8_tbl %>% 
