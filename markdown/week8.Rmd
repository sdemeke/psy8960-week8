...
```{r script_settings, message=FALSE}
setwd(dirname(rstudioapi::getActiveDocumentContext()$path)) #hash this out in binder!
library(tidyverse)
#Instructions "Suppress all output from this chunk but display a copy of executed code." Only output is suppressed message from tidyverse call
```
...
Gender is recoded\ as a factor for more readable filtering in shiny and the two time variables are also recoded as POSIXct/POSIXt variables for easier selection and filtering.
```{r}
week8_tbl %>% 
 mutate(gender = factor(gender,levels=c("M","F"),labels=c("Male","Female"))) %>% 
 mutate(across(timeStart:timeEnd, \(x) ymd_hms(x,truncated = 3))) %>% 
